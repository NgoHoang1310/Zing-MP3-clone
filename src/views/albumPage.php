<!-- content -->

<div class="main-content">
    <div class="library-header">
        <span class="library-header__text text-white d-flex align-items-center">
            <h1 style="font-size: 40px; font-weight: 700;">Album</h1>
            <span class="library-header__icon-play ms-3">
                <i class="fa-solid fa-play text-black fs-4 "></i>
            </span>
        </span>
    </div>

    <div class="library-nav mt-5">
        <ul class="nav nav-underline fs-2">
            <li class="nav-item">
                <a id="albumTab" class="nav-link activeTopBar" href="../views/main.php?page_layout=libraryAlbumPage" >Tất cả</a>
            </li>
        </ul>
    </div>

    <div class="library-empty text-white">
        <div class="library-empty__icon">
            <i class="fa-solid fa-music"></i>
        </div>

        <span class="library-empty__title fs-3">Chưa có album nào</span>

        <div class="library-empty__discover fs-2">Tạo album mới</div>
    </div>
</div>

<!-- content -->